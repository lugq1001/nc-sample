...
object ImageUtil {
    ...
    fun tintDrawable(context: Context, @DrawableRes image: Int, @ColorRes color: Int): Drawable {
        ...
        val wrappedDrawable = DrawableCompat.wrap(drawable)
        val finalColor = ContextCompat.getColor(context, color)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            drawable.colorFilter = BlendModeColorFilter(finalColor, BlendMode.SRC_ATOP)
        } else {
            drawable.setColorFilter(finalColor, PorterDuff.Mode.SRC_ATOP)
        }
        wrappedDrawable.setColorFilter(ContextCompat.getColor(context, color), PorterDuff.Mode.SRC_IN)
        return wrappedDrawable
    }
